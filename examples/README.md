...
For the full schema, see [`developer-toolkit/warranty-policy.md`](../developer-toolkit/warranty-policy.md). For examples that add resource profiles or advanced controls, use Manual Warrant Advanced Mode so the signed source round-trips without field loss. The supported Form and Builder subsets are listed in the [current generated capability matrix](../developer-toolkit/policy-2-2.md#authoring-capability-matrix).
