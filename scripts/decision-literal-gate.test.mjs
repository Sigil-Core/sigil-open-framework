...
try {
  ...
  writeFileSync(resolve(root, 'src/planted.ts'), `export const planted = '${legacy}';\n`);
  ...
  if (blocked.status !== 1 || !blocked.stderr.includes('src/planted.ts:1')) throw new Error(`blocking proof failed\n${blocked.stdout}\n${blocked.stderr}`);
