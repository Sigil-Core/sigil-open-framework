...
The reference signer publishes a JSON Web Key Set (JWKS) for its JWT Intent Attestations. That is the correct discovery format for JWK-formatted JWT verification keys. Other signature systems can publish verification material through formats suited to their protocols, including operator-hosted directories for RFC 9421 HTTP Message Signatures. Those approaches can also support independent verification. SOF's claim is narrower: a third party can discover the verification material and evaluate the evidence without a relationship to the deployment. It does not claim that JWKS is categorically more trustworthy than another discovery format.
